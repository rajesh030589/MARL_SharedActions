...
def plot_figure():
    ...
    f = open('Data/model_seq.pkl', 'rb')
    [Ret_model_seq, Q_mod_seq] = pickle.load(f)
    f.close()

    f = open('Data/model_rl.pkl', 'rb')
    [Ret_model_rl, Q_mod_rl] = pickle.load(f)
    f.close()

    f = open('Data/modelfree_500_rl.pkl', 'rb')
    Ret_modelfree1_rl = pickle.load(f)
    f.close()

    f = open('Data/modelfree_20_rl.pkl', 'rb')
    Ret_modelfree4_rl = pickle.load(f)
    f.close()

    f = open('Data/modelfree_100_rl.pkl', 'rb')
    Ret_modelfree3_rl = pickle.load(f)
    f.close()

    Ret_model_seq = np.flipud(np.squeeze(np.apply_over_axes(np.mean, Ret_model_seq, [1, 2])))
    Ret_mod_seq = Ret_model_seq[0]*np.ones(len(Ret_model_rl))
    ...
    plt.plot(smooth(Ret_model_rl[:, 0], 1000)[:-500], label='Model RL', color='b')
    plt.plot(smooth(Ret_modelfree1_rl[:, 0], 1000)[:-500], label='Model Free RL K = 500', color='g')
    plt.plot(smooth(Ret_modelfree3_rl[:, 0], 1000)[:-500], label='Model Free RL K = 100', color='r')
    plt.plot(smooth(Ret_modelfree4_rl[:, 0], 1000)[:-500], label='Model Free RL K = 20', color='k')
    ...
    # plt.figure()
    # plt.plot(Q_mod_seq[i, j, :])
    # plt.plot(Q_mod_rl[i, j, :])
