...
def model_seq(RunsI, belief_res, TimeI):
    ...
    V = np.zeros((belief_res, belief_res))
    Q = np.zeros((belief_res, belief_res, gamma_size))
    ...
    for t in tqdm(range(TimeI)):
        for b1 in range(belief_res):
            for b2 in range(belief_res):
                ...
                for g_idx in range(gamma_size):
                    ...
                    for i in range(4):
                        ...
                        if t == 0:
                            ...
                            Vt = interpolate.interp2d(belief1, belief2, V[:, :])(next_belief[0], next_belief[1])
                        Rew = Rew + (R + discount*Vt)*belief_prob[i]
                    Q[b1, b2, g_idx] = Rew

                gamma_opt_idx = np.argmax(Q[b1, b2, :])
                ...
                G[b1, b2, 1, 0] = np.random.choice(np.arange(3), p=gamma_opt[1][:, 0])
                G[b1, b2, 1, 1] = np.random.choice(np.arange(3), p=gamma_opt[1][:, 1])

                V[b1, b2] = Q[b1, b2, gamma_opt_idx]

    with open('/home/raj/Dropbox/MARL/Python Codes/Infinite/Data/model_seq.pkl', 'wb') as f:
        pickle.dump([V, G], f)
    f.close()
